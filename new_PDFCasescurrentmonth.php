...
$pdf->SetWidths(array(7,17,27,21,26,25,25,27,19));
	...
	$sql = "SELECT lw_trans.brief_file,
				...
				lw_cases.judgename
	FROM lw_trans LEFT JOIN lw_courts ON lw_trans.courtname=lw_courts.courtid LEFT JOIN lw_cases ON lw_trans.brief_file=lw_cases.brief_file WHERE lw_trans.currtrandate LIKE '%" . $curryear .'-' . $currmonth . '-' . "%'  ORDER BY lw_trans.currtrandate ASC";
...
$pdf->SetXY( 25, 10 );
	$pdf->SetFont('','B','10'); 
	$pdf->Cell(40,20,'Current Month\'s Court Cases List ',15); 
...
$pdf->AliasNbPages();
   //$pdf = $pdf->Output();
	...
	$len = strlen($pdf);
		...
		header('Content-Disposition: inline; filename=CasescurrentmonthList.pdf');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Pragma: public');
		$pdf->Output('CasescurrentmonthList.pdf','I');

		
	} else {
		
		$title = _('Print Current Month\'s Cases List Error'); ?>
