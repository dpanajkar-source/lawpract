...
$pdf->SetWidths(array(7,17,27,21,27,25,25,26,19));
	...
	$sql = 'SELECT lw_trans.brief_file,
			...
			FROM lw_trans LEFT JOIN lw_courts ON lw_trans.courtname=lw_courts.courtid LEFT JOIN lw_cases ON lw_trans.brief_file=lw_cases.brief_file WHERE lw_trans.currtrandate="' . $date->format('Y-m-d') . '" ORDER BY lw_courts.courtname ASC';
...
if($myrowpartyrole['tag']=='L')
	...
	$pdf->Row(array($i++,$Contacts['prevcourtdate'],$Contacts['brief_file'],$myrowcourt['courtname'] . ' , ' . $Contacts['judgename'],$Contacts['courtcaseno'],$party,$oppoparty,$myrowstage['stage'],null));
	}	
	...
	$len = strlen($pdf);
		...
		header('Content-Disposition: inline; filename=CasestomorrowList.pdf');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Pragma: public');
		$pdf->Output('CasestomorrowList.pdf','I');

		
	} else {
		
		$title = _('Print Cases Tomorrow List Error'); ?>
		<div style="text-align:center; font-weight:bold">There were no cases tomorrow to print out for the selections specified
