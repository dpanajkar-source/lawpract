 ...
 */     
function MM_openbrwindow(x,width,height,idnew){
	
window.open(x + '?idnew=' + idnew,'popup','width=' + width + ',height=' + height + ',scrollbars=0,resizable=no,toolbar=no,directories=no,location=no,menubar=no,status=no,left=180,top=30');
}		
...
MM_openbrwindow('bf_new_PDFAgentReceipt.php',600,400,editableGrid.getCell(rowIndex, 0).innerHTML);

editableGrid.setValueAt(rowIndex, 13, oldValue)==false;

}else
{
	$.ajax({
	url: 'pr_update_searchtitle.php',
		type: 'POST',
		dataType: "html",
	   		data: {
...
{ 
	this.editableGrid = new EditableGrid("lw_searchtitle", {
		enableSort: true,
...
DatabaseGrid.prototype.fetchGrid = function()  {
	...
	this.editableGrid.loadJSON("pr_searchtitle_loaddata.php?db_tablename=lw_searchtitle");
  ...
  var self = this;
    ...
    if($("#Courtcasenohidden").val()=='')
		...
		data: {
			...
			propertydetails:  $("#Details").val(),
			status:  $("#Status").val(),
			agentcharges:  $("#Agentcharges").val(),
			agent:  $("#Agent").val(),			
			searchfees:  $("#Searchfees").val(),
			feescharged:  $("#Feescharged").val(),
