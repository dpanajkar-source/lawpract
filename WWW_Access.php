...
if (isset($_POST['submit']) || isset($_GET['remove']) || isset($_GET['add']) ) {
	...
	// if $_POST['SecRoleName'] then it is a modification on a SecRole
...
/* If its the first time the page has been displayed with no parameters then none of the above are true and the list of Users will be displayed with links to delete or edit each. These will call the same page again and allow update/input or deletion of the records*/
	...
	echo "<thead><tr><td>" . _('Role') . "</td><td></td><td></td></tr></thead>";
...
if (isset($SelectedRole)) {
echo '<div  id="form_containeraccess" >';
	echo "<div class='centre'><a href='" . $_SERVER['PHP_SELF'] ."?" . SID . "'><label>" . _('Review Existing Roles') . '</label></a></div>';
}
...
echo '<tr><td><label>' . _('Role') . ":</label></td>
	<td><input type='text' name='SecRoleName' class='md-input' size=40 maxlength=40 VALUE='" . $_POST['SecRoleName'] . "'></tr>";
echo "</table><br>
	<div><input type='Submit' name='submit' value='" . _('Enter Role') . "' class='md-btn md-btn-primary'></div></form>";
...
if (isset($SelectedRole)) {
	...
	echo '<table align="center" class="uk-table"><tr>';
