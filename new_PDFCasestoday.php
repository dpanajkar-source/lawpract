<?php
	...
	$sql = 'SELECT lw_trans.brief_file,
				...
				lw_cases.judgename
		FROM lw_trans LEFT JOIN lw_cases ON lw_trans.brief_file=lw_cases.brief_file WHERE currtrandate="' . $date->format('Y-m-d') . '" ORDER BY lw_trans.courtname ASC';
...
if ($myrownum_rows>0){
	...
	$len = strlen($pdf);
		...
		header('Content-Disposition: inline; filename=CasestodayList.pdf');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Pragma: public');
		$pdf->Output('CasestodayList.pdf','I');

		
	} else {
		
		$title = _('Print Cases Today List Error');?>
		<div style="text-align:center; font-weight:bold">There were no cases today to print out for the selections specified
