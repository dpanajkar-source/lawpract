...
include('includes/PDFStarter.php');
	...
	$PageNumber = 0;
	$FontSize=10;
	$pdf->addinfo('Title', _('Receipt'));
	$pdf->addinfo('Subject', _('Receipt'));
	...
	if (DB_error_no($db) !=0) {
	  $title = _('Transaction Details') . ' - ' . _('Problem Report') . '....';
	   ...
	   exit;
			...
			include('includes/PDFReceiptPageHeader.php');
			$FontSize=10;
...
$pdf->selectFont('./fonts/Helvetica.afm');
				$LeftOvers = $pdf->addTextWrap(300,$YPos+10,1000,$FontSize,': ' . $myrowpartydetails['name']);
...
$pdf->selectFont('./fonts/Helvetica.afm');
                $LeftOvers = $pdf->addTextWrap(300,$YPos-10,300,$FontSize,': ' . number_word(-($myrowpartytransdetails['amountreceived'])));
 $pdf->SetFont('Helvetica','BI',16);
...
$pdf->selectFont('./fonts/Helvetica.afm');
				$LeftOvers = $pdf->addTextWrap(300,$YPos-70,300,$FontSize,': ' . $myrowpartytransdetails['brief_file']);
$pdf->SetFont('Helvetica','BI',16);			 
				$LeftOvers = $pdf->addTextWrap(120,$YPos-100,150,$FontSize, _('CASE NO'),'left');
$pdf->selectFont('./fonts/Helvetica.afm');
				$LeftOvers = $pdf->addTextWrap(300,$YPos-100,200,$FontSize,': ' . $myrowpartyeconomydetails['courtcaseno']);
 $pdf->SetFont('Helvetica','BI',16);
				$LeftOvers = $pdf->addTextWrap(120,$YPos-130,900,$FontSize, _('PURPOSE OF PAYMENT'),'left');
$pdf->selectFont('./fonts/Helvetica.afm');
				$LeftOvers = $pdf->addTextWrap(300,$YPos-130,900,$FontSize,': ' . $myrowpartytransdetails['narration']);
...
$LeftOvers = $pdf->addTextWrap(120,$YPos-130,300,$FontSize, _('This receipt is valid subject to Realisation of Cheque if Applicable.'),'left');
	...
	$len = strlen($pdfcode);
      ...
      if ($len<=20){
		$title = _('Print Party Dues Error');
		echo '<p>';
		...
		header('Content-Disposition: inline; filename=PartyDues.pdf');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Pragma: public');

		$pdf->Output('PDFReceiptPageHeader.pdf','I');
