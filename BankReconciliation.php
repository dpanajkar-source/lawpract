...
if (isset($_POST['PostExchangeDifference']) and is_numeric($_POST['DoExchangeDifference'])){
	...
	if (!is_numeric($_POST['BankStatmentBalance'])){
		prnMsg('The entry in the bank statement balance is not numeric. The balance on the bank statement should be entered. The exchange difference has not been calculated and no general ledger journal has been created','warn');
		...
		$ErrMsg = 'Could not retrieve the exchange rate for the selected bank account';
...
//yet to code the journal
		...
		$SQL = 'INSERT INTO gltrans (type,
								  ...
								  VALUES (36,
									...
									'" . $BankAccountName . ' ' . 'reconciliation on' . ' ' . Date($_SESSION['DefaultDateFormat']) . "'," . $ExchangeDifference . ')';

		$ErrMsg = 'Cannot insert a GL entry for the exchange difference because';
		$DbgMsg = 'The SQL that failed to insert the exchange difference GL entry was';
		...
		$SQL = 'INSERT INTO gltrans (type,
								  ...
								  VALUES (36,
									...
									" . $PeriodNo . ',
									' . $_POST['BankAccount'] . ",
									'" . $BankAccountName . ' ' . 'reconciliation on' . ' ' . Date($_SESSION['DefaultDateFormat']) . "',
									" . (-$ExchangeDifference) . ')';

		$result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);

		$result = DB_Txn_Commit($db);
		prnMsg('Exchange difference of' . ' ' . number_format($ExchangeDifference,2) . ' ' . 'has been posted','success');
	} //end if the bank statement balance was numeric
}

echo '<div class="uk-overflow-container">';
  
    /*--------------------Heading Bank Reconciliation here-----------------*/

$SQL = 'SELECT bankaccountname, accountcode FROM bankaccounts';

$ErrMsg = 'The bank accounts could not be retrieved by the SQL because';
...
echo '<div class="uk-width-medium-1-1" style="padding-bottom:10px; padding-top:10px"> <select tabindex="1" name="BankAccount" class="md-input">';

if (DB_num_rows($AccountsResults)==0){
	echo '</select><p>' . 'Bank Accounts have not yet been defined' . '. ' . 'You must first' . "<a href='" . $rootpath . "/lw_bankaccounts_alt.php'>" . ' define the bank accounts' . '</a>' . ' ' . 'and general ledger accounts to be affected' . '.';
...
echo '<div class="uk-width-medium-1-2" style="padding-bottom:10px; padding-top:10px"><input type=submit tabindex="2" name="ShowRec" class="md-btn md-btn-primary" value="Show Bank Reconciliation Statement"></div>';

// --------------user submits show Reconciliation Statement---------------------
...
/*Get the balance of the bank account concerned */
	...
	$ErrMsg = 'The bank account balance could not be returned by the SQL because';
	...
	$SQL = 'SELECT rate,
				...
				ON bankaccounts.currcode=currencies.currabrev
				WHERE bankaccounts.accountcode = ' . $_POST['BankAccount'];
	$ErrMsg = 'Could not retrieve the currency and exchange rate for the selected bank account';
	$CurrencyResult = DB_query($SQL,$db);
	$CurrencyRow =  DB_fetch_row($CurrencyResult);
	$ExRate = $CurrencyRow[0];
 ...
 echo '<div class="uk-overflow-container">';
	...
	echo '<br>Bank Reconciliation';
	echo '<table align="center" class="uk-table" border=1>
			<tr><td colspan=6><b>' . $BankAccountName . ' ' . 'Balance as at' . ' ' . Date($_SESSION['DefaultDateFormat']);
	...
	$SQL = 'SELECT amount/exrate AS amt,
				...
				AND banktrans.type=13
				AND ABS((amount/exrate)-amountcleared)>0.009 ORDER BY transdate';

	echo '<tr></tr>'; /*Bang in a blank line */

	$ErrMsg = 'The unpresented cheques could not be retrieved by the SQL because';
	...
	//end of while loop
	echo '<tr></tr>
			<tr><td colspan=6>' . 'Total of all unpresented cheques' . '</td><td align=right>' . number_format($TotalUnpresentedCheques,2) . '</td></tr>';
	...
	$ErrMsg = 'The uncleared deposits could not be retrieved by the SQL because';

	$UPChequesResult = DB_query($SQL,$db,$ErrMsg);

	echo '<tr><td colspan=6><b>' . 'Less deposits not cleared' . ':</b></td></tr>';
	...
	if ($_SESSION['CompanyRecord']['currencydefault']!=$BankCurrCode AND !isset($_POST['DoExchangeDifference'])){
		...
		echo '<tr><td colspan=7><hr></td></tr>
				<tr><td colspan=7>' . 'It is normal for foreign currency accounts to have exchange differences that need to be reflected as the exchange rate varies. This reconciliation is prepared using the exchange rate set up in the currencies table (see the set-up tab). This table must be maintained with the current exchange rate before running the reconciliation. If you wish to create a journal to reflect the exchange difference based on the current exchange rate to correct the reconciliation to the actual bank statement balance click below.' . '</td></tr>';
		echo '<tr><td colspan=7 align="center"><input type=submit name="DoExchangeDifference" value="' . 'Calculate and Post Exchange Difference' . '"></td></tr>';
	}
	echo '</table>';
}

echo '<div class="uk-width-medium-1-2" style="padding-bottom:10px"><a tabindex="4" href="' . $rootpath . '/lw_bankmatching_alt.php?' . SID . '&Type=Payments">' . '(Match off cleared payments)' . '</a>';
echo '&nbsp;&nbsp;&nbsp;&nbsp;<a tabindex="5" href="' . $rootpath . '/lw_bankmatching_alt.php?' . SID . '&Type=Receipts">' . '(Match off cleared deposits)' . '</a>';
echo '</form></div>';
