...
if (isset($_POST['JournalProcessDate'])){
	...
	if (!Is_Date($_POST['JournalProcessDate'])){
		prnMsg('The date entered was not valid please enter the date to process the journal in the format'. $_SESSION['DefaultDateFormat'],'warn');
...
if (isset($_POST['CommitBatch']) and $_POST['CommitBatch']=='Accept and Process Journal'){
...
*/
	...
	foreach ($_SESSION['JournalDetail']->GLEntries as $JournalItem) {
		...
		$SQL= $SQL . 'VALUES (0,
					...
					" . $JournalItem->Amount .
					",'".$JournalItem->tag."')";
		$ErrMsg = 'Cannot insert a GL entry for the journal line because';
		...
		if ($_POST['JournalType']==_('Reversing')){
			...
			$ErrMsg ='Cannot insert a GL entry for the reversing journal because';
			$DbgMsg = 'The SQL that failed to insert the GL Trans record was';
			$result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);
	...
	prnMsg('Journal'.' ' . $TransNo . ' '.'has been successfully entered','success');
	...
	$AllowThisPosting = true; //by default
		if ($_SESSION['ProhibitJournalsToControlAccounts'] == 1){
			...
			if ($_SESSION['CompanyRecord']['gllink_creditors'] == '1' AND $_POST['GLManualCode'] == $_SESSION['CompanyRecord']['creditorsact']){
				prnMsg('GL Journals involving the creditors control account cannot be entered. The general ledger creditors ledger (AP) integration is enabled so control accounts are automatically maintained by RosERP. This setting can be disabled in System Configuration','warn');
				$AllowThisPosting = false;
			}
		}
		if (in_array($_POST['GLManualCode'], $_SESSION['JournalDetail']->BankAccounts)) {
			prnMsg('GL Journals involving a bank account cannot be entered' . '. ' . 'Bank account general ledger entries must be entered by either a bank account receipt or a bank account payment','info');
			$AllowThisPosting = false;
		}
		...
		if ($AllowThisPosting) {
			...
			if (DB_num_rows($Result)==0){
				prnMsg('The manual GL code entered does not exist in the database' . ' - ' . 'so this GL analysis item could not be added. Please create GL account first to use','warn');
		...
		if ($_SESSION['ProhibitJournalsToControlAccounts'] == 1){
			if ($_SESSION['CompanyRecord']['gllink_debtors'] == '1' AND $_POST['GLCode'] == $_SESSION['CompanyRecord']['debtorsact']){
				prnMsg('GL Journals involving the debtors control account cannot be entered. The general ledger debtors ledger (AR) integration is enabled so control accounts are automatically maintained by RosERP. This setting can be disabled in System Configuration','warn');
				$AllowThisPosting = false;
			}
		...
		if (in_array($_POST['GLCode'], $_SESSION['JournalDetail']->BankAccounts)) {
			prnMsg('GL Journals involving a bank account cannot be entered' . '. ' . 'Bank account general ledger entries must be entered by either a bank account receipt or a bank account payment','warn');
			$AllowThisPosting = false;
		}
...
echo '<p><table border=0 width=100%>
	<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/maintenance.gif" title="' . 'Search' . '" alt="">' . ' ' . $title.'<tr><hr></tr>';
...
if (!Is_Date($_SESSION['JournalDetail']->JnlDate)){
	...
	echo '<tr>
			<td colspan=5><table border=0 width=30%><tr><td>'.'Date to Process Journal'.":</td>
	...
	echo '<th>' . 'GL Account Code' . '</th>';
	echo '<th>' . 'Select GL Account' . '</th></tr>';

/* Set up form for the transaction entry for a GL Payment Analysis item */

echo '<div class="centre"><font size=3 color=blue>' . 'Journal Line Entry' . '</font></div>';
...
// End select tag
	...
	$result=DB_query($sql, $db);
	echo '<td> <img src="css/professional/images/icon_arrow.gif"/><select name="GLCode" onChange="return assignComboToInput(this,'.'GLManualCode'.')">';
	while ($myrow=DB_fetch_array($result)){
	...
	echo '</tr><tr><th>' . 'Debit' . "</th>".'<td><input type=Text class="number" Name = "Debit" ' .
				'onChange="eitherOr(this, '.'Credit'.')"'.
				'Maxlength=12 size=10 value=' . $_POST['Debit'] . '> <i>Can enter only one value- Debit or Credit</i></td>';
	echo '</tr><tr><th>' . 'Credit' . "</th>".'<td><input type=Text class="number" Name = "Credit" ' .
				'onChange="eitherOr(this, '.'Debit'.')"'.
				'Maxlength=12 size=10 value=' . $_POST['Credit'] . '></td>';
	echo '</tr><tr><td></td><td></td><th>'. 'Narrative'. '</th>';
	echo '</tr><tr><th></th><th>' . 'GL Narrative' . "</th>";
	...
	echo "<div class='centre'><input type=submit name=Process value='" . 'Accept' . "'></div><br><hr><br>";
...
if (ABS($_SESSION['JournalDetail']->JournalTotal)<0.001 AND $_SESSION['JournalDetail']->GLItemCounter > 0){
	echo "<br><br><div class='centre'><input type=submit name='CommitBatch' value='".'Accept and Process Journal'."'></div>";
} elseif(count($_SESSION['JournalDetail']->GLEntries)>0) {
	echo '<br><br>';
	prnMsg('The journal must balance ie debits equal to credits before it can be processed','warn');
}
