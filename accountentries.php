...
echo "<input type='text' class='md-input' alt='".$_SESSION['DefaultDateFormat']."' name='JournalProcessDate' maxlength=10 size=11 data-uk-datepicker=\"{format:'DD/MM/YYYY'}\" placeholder='Date to Process Entry'></div>";
	
	echo '<div class="uk-width-medium-1-2" style="padding-bottom:10px">Entry Type<select name=JournalType class="md-input">';
...
// End select tag
	...
	if (!isset($_POST['GLManualCode'])) {
		$_POST['GLManualCode']='';
            ...
            echo '<div class="uk-width-medium-1-2" style="padding-bottom:10px">Gl Code<input type=Text Name="GLManualCode" class="md-input" Maxlength=12 size=12 onChange="inArray(this.value, GLCode.options,'.
		"'".'The account code '."'".'+ this.value+ '."'".' doesnt exist'."'".')"' .
			' VALUE='. $_POST['GLManualCode'] .'  ></div>';
	...
	echo ' <div class="uk-width-medium-1-2" style="padding-bottom:10px">Debit<input type=Text class="md-input" Name = "Debit" ' .
				'onChange="eitherOr(this, '.'Credit'.')"'.
				'Maxlength=12 size=10 value=' . $_POST['Debit'] . '></div>';
	echo ' <div class="uk-width-medium-1-2" style="padding-bottom:10px">Credit<input type=Text class="md-input" Name = "Credit" ' .
				'onChange="eitherOr(this, '.'Debit'.')"'.
				'Maxlength=12 size=10 value=' . $_POST['Credit'] . '></div>';
	...
	echo "<table width=80% style='text-align:center; margin-left:100px'><tr><td><table width=100%>
					<tr>
						<th>".'GL Tag'."</th>
						<th>".'GL Account'."</th>
						<th>".'Debit'."</th>
...
if (ABS($_SESSION['JournalDetail']->JournalTotal)<0.001 AND $_SESSION['JournalDetail']->GLItemCounter > 0){
	echo "<br><br><div class='centre'><input type=submit name='CommitBatch' value='".'Accept and Process Entry'."'></div></div>";
