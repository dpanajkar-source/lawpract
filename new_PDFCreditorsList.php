...
require('mc_tablenew_SupplierList.php');
	...
	$pdf->SetXY( 10, 10 ); 
	$pdf->SetFont('','B','18'); 
	$pdf->Cell(40,10,'Creditors List',15); 
	$pdf->Ln(); 
	...
	$result=DB_query("SELECT id,
			...
			mobile,
			email
		FROM lw_contacts_other ORDER BY name ASC",$db);
...
$arr=array();
	...
	$len = strlen($pdfcode);
      ...
      if ($len<=20){
		$title = _('Print Creditors List Error');
	
		echo '<p>';
		prnMsg( _('There were no Creditors to print out for the selections specified') );
		...
		header('Content-Disposition: inline; filename=CreditorsList.pdf');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Pragma: public');
		$pdf->Output('CreditorsList.pdf','I');
