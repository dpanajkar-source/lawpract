...
if (!isset($_GET['Delete']) AND isset($_SESSION['ReceiptBatch'])){ //always process a header update unless deleting an item
	...
	$ErrMsg ='The bank account name cannot be retrieved because';
	...
	if (DB_num_rows($result)==1){
		...
		prnMsg( 'The bank account number' . ' ' . $_POST['BankAccount'] . ' ' . 'is not set up as a bank account','error');
	...
	if (isset($_POST['ExRate']) and $_POST['ExRate']!=''){
		if (is_numeric($_POST['ExRate'])){
			...
			prnMsg('The exchange rate entered should be numeric','warn');
		}
	}
	if (isset($_POST['FunctionalExRate']) and $_POST['FunctionalExRate']!=''){
		if (is_numeric($_POST['FunctionalExRate'])){
			...
			prnMsg('The functional exchange rate entered should be numeric','warn');
		}
	}
	...
	if ($_SESSION['ReceiptBatch']->Currency!=$_POST['Currency']){
		...
		if (count($_SESSION['ReceiptBatch']->Items)>0){
			unset($_SESSION['ReceiptBatch']->Items);
			prnMsg('Changing the currency of the receipt means that existing entries need to be re-done - only customers trading in the selected currency can be selected','warn');
		}
...
if (isset($_POST['CommitBatch'])){
 ...
 /* once all receipts items entered, process all the data in the
  ...
  of all receipts in the batch and PartyTrans records for each receipt item
  all PartyTrans will refer to a single banktrans. A GL entry is created for
  ...
  also add the total discount total receipts*/
	...
	if ($_SESSION['CompanyRecord']==0){
		prnMsg('The company has not yet been set up properly' . ' - ' . 'this information is needed to process the batch' . '. ' . 'Processing has been cancelled','error');
		...
		if ($ReceiptItem->GLCode !=''){
			if ($_SESSION['CompanyRecord']['gllink_debtors']==1){ /* then enter a GLTrans record */
				 $SQL = 'INSERT INTO gltrans (type,
					...
					VALUES (12,
						...
						'" . $ReceiptItem->tag . "'" . ')';
				$ErrMsg = 'Cannot insert a GL entry for the receipt because';
				$DbgMsg = 'The SQL that failed to insert the receipt GL entry was';
			...
			/*Need to deal with the case where the payment from one bank account could be to a bank account in another currency */
				...
				*/
				...
				$ErrMsg = 'Cannot insert a bank transaction using the SQL';
				$result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);
			...
			$SQL = 'INSERT INTO lw_PartyTrans (transno,
							type,
							debtorno,
							branchcode,
							trandate,
			...
			$DbgMsg = 'The SQL that failed to insert the customer receipt transaction was';
			$ErrMsg = 'Cannot insert a receipt transaction against the customer because' ;
			$result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);
			...
			$DbgMsg = 'The SQL that failed to update the date of the last payment received was';
			$ErrMsg = 'Cannot update the customer record for the date of the last payment received because';
			$result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);
	...
	if ($_SESSION['CompanyRecord']['gllink_debtors']==1){ /* then enter GLTrans records for discount, bank and debtors */
		...
		if ($BatchReceiptsTotal!=0){
			...
			$SQL="INSERT INTO gltrans (type,
				...
				VALUES (12,
					...
					" . $BatchReceiptsTotal . ')';
			$DbgMsg = 'The SQL that failed to insert the GL transaction fro the bank account debit was';
			$ErrMsg = 'Cannot insert a GL transaction for the bank account debit';
			$result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);
		...
		if ($BatchDebtorTotal!=0){
			...
			$DbgMsg = 'The SQL that failed to insert the GL transaction for the debtors account credit was';
			$ErrMsg = 'Cannot insert a GL transaction for the debtors account credit';
			$result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);
		...
		if ($BatchDiscount!=0){
			...
			$SQL='INSERT INTO gltrans ( type,
				...
				VALUES (12,
					...
					'" . $_SESSION['ReceiptBatch']->Narrative . "',
					" . $BatchDiscount . ')';
			$DbgMsg = 'The SQL that failed to insert the GL transaction for the payment discount debit was';
			$ErrMsg = 'Cannot insert a GL transaction for the payment discount debit';
			$result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);
	...
	prnMsg( 'Receipt batch' . ' ' . $_SESSION['ReceiptBatch']->BatchNo . ' ' . 'has been successfully entered into the database','success');

	echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/printer.png" title="' . 'Print' . '" alt="">' . ' ' . '<a href="' . $rootpath . '/PDFBankingSummary.php?BatchNo=' . $_SESSION['ReceiptBatch']->BatchNo . '">' . 'Print PDF Batch Summary' . '</a></p>';
	echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/allocation.png" title="' . 'Allocate' . '" alt="">' . ' ' . '<a href="' . $rootpath . '/CustomerAllocations.php">' . 'Allocate Receipts' . '</a></p>';
	unset($_SESSION['ReceiptBatch']);
...
/*Will only be true if clicked to search for a customer code */
	...
	if ($_POST['Keywords']=="" AND $_POST['CustCode']=="" AND $_POST['CustInvNo']=="") {
		...
		if (DB_error_no($db) !=0) {
			prnMsg('The searched customer records requested cannot be retrieved because' . ' - ' . DB_error_msg($db),'error');
			if ($debug==1){
				prnMsg('SQL used to retrieve the customer details was' . '<br>' . $sql,'error');
			}
			...
			prnMsg( 'No customer records contain the selected text' . ' - ' . 'please alter your search criteria and try again','info');
		}
...
customer record returned by the search - this record is then auto selected */
	...
	$ErrMsg = 'The customer details could not be retrieved because';
	$DbgMsg = 'The SQL that failed was';
...
echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/transactions.gif" title="' . 'Enter Receipt' . '" alt="">' . ' ' . 'Enter Customer Receipt' . '</p>';
echo '<div class="page_help_text">' . 'To enter a payment TO a customer (ie. to pay out a credit note), enter a negative payment amount.' . '</div>';
echo '<br><table><tr><td>' . 'Bank Account' . ':</td>
				 <td><select tabindex=1 name="BankAccount" onChange="ReloadForm(form1.BatchInput)">';

if (DB_num_rows($AccountsResults)==0){
	echo '</select></td></tr></table><p>';
	prnMsg('Bank Accounts have not yet been defined' . '. ' . 'You must first' . ' ' . '<a href="' . $rootpath . '/BankAccounts.php">' . 'define the bank accounts' . '</a>' . 'and general ledger accounts to be affected','info');
...
if (DB_num_rows($result)==0){
	echo '</select></td></tr>';
	prnMsg('No currencies are defined yet' . '. ' . 'Receipts cannot be entered until a currency is defined','warn');
...
if ($_SESSION['ReceiptBatch']->AccountCurrency!=$_SESSION['ReceiptBatch']->Currency AND isset($_SESSION['ReceiptBatch']->AccountCurrency)){
	if (isset($SuggestedExRate)){
		$SuggestedExRateText = '<b>' . 'Suggested rate:' . ' ' . number_format($SuggestedExRate,4) . '</b>';
	} else {
		$SuggestedExRateText ='';
	}
	...
	echo '<tr><td>' . 'Receipt Exchange Rate' . ':</td>
			<td><input tabindex=4 type="text" name="ExRate" maxlength=10 size=12 class=number value="' . $_SESSION['ReceiptBatch']->ExRate . '"></td>
			<td>' . $SuggestedExRateText . ' <i>' . 'The exchange rate between the currency of the bank account currency and the currency of the receipt' . '. 1 ' . $_SESSION['ReceiptBatch']->AccountCurrency . ' = ? ' . $_SESSION['ReceiptBatch']->Currency . '</i></td></tr>';
}

if ($_SESSION['ReceiptBatch']->AccountCurrency!=$_SESSION['CompanyRecord']['currencydefault']
											AND isset($_SESSION['ReceiptBatch']->AccountCurrency)){
	if (isset($SuggestedFunctionalExRate)){
		$SuggestedFunctionalExRateText = '<b>' . 'Suggested rate:' . ' ' . number_format($SuggestedFunctionalExRate,4) . '</b>';
	} else {
	...
	echo '<tr><td>' . 'Functional Exchange Rate' . ':</td><td><input tabindex=5 type="text" name="FunctionalExRate" class=number maxlength=10 size=12 value="' . $_SESSION['ReceiptBatch']->FunctionalExRate . '"></td>
			<td>' . ' ' . $SuggestedFunctionalExRateText . ' <i>' . 'The exchange rate between the currency of the business (the functional currency) and the currency of the bank account' .  '. 1 ' . $_SESSION['CompanyRecord']['currencydefault'] . ' = ? ' . $_SESSION['ReceiptBatch']->AccountCurrency . '</i></td></tr>';
}

echo '<tr><td>' . 'Receipt Type' . ":</td><td><select tabindex=6 name=ReceiptType>";
...
echo '<tr><td>' . 'Narrative' . ':</td><td><input tabindex=7 type="text" name="BatchNarrative" maxlength=50 size=52 value="' . $_SESSION['ReceiptBatch']->Narrative . '"></td></tr>';
echo '<tr><td colspan=3><div class="centre"><input tabindex=8 type=submit name="BatchInput" Value="' . 'Accept' . '"></div></td></tr>';
echo '</table>
		<hr>';
...
if (isset($_SESSION['ReceiptBatch'])){
	...
	echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/transactions.gif" title="' . 'Banked' . '" alt="">' . ' ' . $_SESSION['ReceiptBatch']->ReceiptType . ' - ' . 'Banked into the' . " " . $_SESSION['ReceiptBatch']->BankAccountName . ' ' . 'on' . ' ' . $_SESSION['ReceiptBatch']->DateBanked . '</p>';
	...
	foreach ($_SESSION['ReceiptBatch']->Items as $ReceiptItem) {
		...
		echo '<tr>
				...
				<td><a href='" . $_SERVER['PHP_SELF'] . '?' . SID . '&Delete=' . $ReceiptItem->ID . "'>" . 'Delete' . '</a></td>
			</tr>';
...
if (isset($_SESSION['CustomerRecord'])
	...
	prnMsg('The selected customer does not trade in the currency of the receipt being entered - either the currency of the receipt needs to be changed or a different customer selected','warn');
	unset($_SESSION['CustomerRecord']);
}
...
/*a customer is selected  */
	...
	echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/customer.png" title="' . 'Customer' . '" alt="">' . ' ' . $_SESSION['CustomerRecord']['name'] . ' - (' . 'All amounts stated in' . ' ' . $_SESSION['CustomerRecord']['currency'] . ')' . 'Terms' . ': ' . $_SESSION['CustomerRecord']['terms'] . '<br/>' . 'Credit Limit' . ': ' . number_format($_SESSION['CustomerRecord']['creditlimit'],0) . '  ' . 'Credit Status' . ': ' . $_SESSION['CustomerRecord']['reasondescription'];
	...
	echo '<table width="90%" border="1">
			<tr>
				...
				<th>' . 'Current' . '</th>
				<th>' . 'Now Due' . '</th>
				<th>' . $_SESSION['PastDueDays1'] . '-' . $_SESSION['PastDueDays2'] . ' ' . 'Days Overdue' . '</th>
				<th>' . 'Over' . ' ' . $_SESSION['PastDueDays2'] . ' ' . 'Days Overdue' . '</th>
			</tr>';
...
/* Set up a heading for the transaction entry for a GL Receipt */
	...
	echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/transactions.gif" title="' . _('Banked') . '" alt=""> ' . _('General Ledger Receipt Entry') . '</font><table>';

	//Select the tag
	echo '<tr><td>' . 'Select Tag' . ':</td><td><select name="tag">';
...
// End select tag
	...
	echo '<tr><td>' . 'GL Account' . ':</td><td><select tabindex=8 name="GLCode">';
...
if (((isset($_SESSION['CustomerRecord'])
	...
	if (!isset($_POST['GLEntry'])){
		echo '<tr><td>' . 'Amount of Discount' . ':</td>
			<td><input tabindex=10 type="text" name="Discount" maxlength=12 size=13 class=number value="' . $_POST['Discount'] . '"> ' . 'agreed prompt payment discount is' . ' ' . $DisplayDiscountPercent . '</td></tr>';
	} else {
		echo '<input tabindex=11 type="hidden" name="Discount" Value=0>';
	}

	echo '<tr><td>' . 'Payee Bank Details' . ':</td>
		<td><input tabindex=12 type="text" name="PayeeBankDetail" maxlength=22 size=20 value="' . $_POST['PayeeBankDetail'] . '"></td></tr>';
	echo '<tr><td>' . 'Narrative' . ':</td>
		<td><input tabindex=13 type="text" name="Narrative" maxlength=30 size=32 value="' . $_POST['Narrative'] . '"></td></tr>';
	echo '</table>';
	echo '<div class="centre"><input tabindex=14 type="submit" name="Process" value="' . 'Accept' . '">';
	echo '<input tabindex=14 type="submit" name="Cancel" value="' . 'Cancel' . '"></div>';
	...
	echo $msg;
	echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/customer.png" title="' . 'Customer' . '" alt="">' . ' ' . 'Select a Customer' . '</b>';
	echo '<table cellpadding=3 colspan=4>';
	echo '<tr><td>' . 'Text in the Customer' . ' ' . '<b>' . 'name' . '</b>:</td>';
	echo '<td><input tabindex=9 type="text" name="Keywords" size=15 maxlength=25></td>';
	echo '<td><font size=3><b>' . 'OR' . '</b></font></td>';
	echo '<td>' . 'Text extract in the Customer' . ' ' . '<b>' . 'code' . '</b>:</td>';
	echo '<td><input tabindex=10 type="text" name="CustCode" size=10 maxlength=18></td>';
	echo '<td><font size=3><b>' . 'OR' . '</b></font></td>';
	echo '<td>' . 'Customer invoice number' . ':</td>';
	echo '<td><input tabindex=11 type="text" name="CustInvNo" size=8 maxlength=8></td>';
	echo '</tr></table>';
	echo '<div class="centre">';
	echo '<input tabindex=11 type=submit name="Search" value="' . 'Search Now' . '">';
	echo '<br/><br/><input tabindex=12 type=submit name="GLEntry" value="' . 'Enter A GL Receipt' . '">';

	if (count($_SESSION['ReceiptBatch']->Items) > 0){
		echo '<br/><br/><input tabindex=13 type=submit name="CommitBatch" VALUE="' . 'Accept and Process Batch' . '"></div>';
	}

	if (isset($CustomerSearchResult)) {

		echo '<table cellpadding=2 colspan=7 border=2>';
		$TableHeader = '<tr><th>' . 'Code' . '</th>
