...
$pdf->Row(array('ID','Notice No','Party','Opposite Party','Notice Date','Send by','Recpt No','Recpt Dt','Claim Dt','Ret. Envelope Date','Received By','Fees','Postage','Other Charges','Remark'));	
	...
	$result=DB_query("SELECT cr.noticeid,
			...
			lw_notices.remark
			FROM lw_noticecr AS cr INNER JOIN lw_notices ON cr.noticeid=lw_notices.noticeno INNER JOIN lw_contacts AS p1 ON cr.party=p1.id INNER JOIN lw_contacts AS p2 ON cr.oppoparty=p2.id INNER JOIN lw_noticereply ON cr.noticeid=lw_noticereply.noticeno WHERE lw_notices.noticedt LIKE '%" . $curryear .'-' . $currmonth . '-' . "%' OR lw_noticereply.replydt LIKE '%" . $curryear .'-' . $currmonth . '-' . "%' ",$db);			
			
		$resulttotalfees=DB_query("SELECT SUM(lw_notices.noticefees) AS totalfees,SUM(lw_notices.postage) AS totalpostage,SUM(lw_notices.othercharges) AS totalothercharges FROM lw_notices WHERE lw_notices.noticedt LIKE '%" . $curryear .'-' . $currmonth . '-' . "%'",$db);
