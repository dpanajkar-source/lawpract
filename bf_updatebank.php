...
if ( $stmt = $mysqli->prepare("UPDATE bf_cases SET ".$colname." = ? WHERE id = ?")) {
...
if($colname=='loanamount' && $value!='')
		{

if ( $stmt = $mysqli->prepare("UPDATE bf_inward SET loanamount= ? WHERE id = ?")) {
	$stmt->bind_param("ss",$value,$id);
	$return = $stmt->execute();
	$stmt->close();
	$query=$stmt;		
...
if($colname=='fees' && $value!='')
		{

if ( $stmt = $mysqli->prepare("UPDATE bf_inward SET fees= ? WHERE id = ?")) {
	$stmt->bind_param("ss",$value,$id);
	$return = $stmt->execute();
	$stmt->close();
	$query=$stmt;		
